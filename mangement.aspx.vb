...
End Function
    ...
    Protected Sub SubmitOrder_Click(sender As Object, e As EventArgs)
        ...
        Try
            ...
            cmd.Connection = conn
            cmd.ExecuteNonQuery()
            sendEmail(ItemQualityTextBox.Text)
...
Protected Sub sendEmail(str As Integer)
    Dim content As String = "<div style='font-size:19px;font-family:arial;'>"
    content += "Dear Madam/Sir, <br><br>"
    content += "A submission has been created in Purchase System!  <span style='color:red'>The order ID is " + str.ToString() + ".</span><br/><br>"
    ...
    mail.From = New MailAddress("qehpurchasesystem@gmail.com")
    mail.To.Add(sysAdminEmail)
    mail.Subject = "Purchase System"
    mail.Body = content

    Dim smtp As New SmtpClient("MAILCORPHTS.server.ha.org.hk", 25)
